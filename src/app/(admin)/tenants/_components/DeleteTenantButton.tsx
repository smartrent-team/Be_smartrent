...
export function DeleteTenantButton({
  ...
  const handleDelete = async () => {
    setLoading(true)
    try {
      await deleteTenantAction(id, userId)
      toast.success(`Đã xóa hồ sơ khách thuê "${name}" thành công!`)
      setOpen(false)
      router.refresh()
    } catch (error: unknown) {
      const errMsg = error instanceof Error ? error.message : 'Không thể xóa khách thuê'
      toast.error(errMsg)
    } finally {
      setLoading(false)
    }
        ...
        <DialogContent className="sm:max-w-[420px] border border-gray-100/50 backdrop-blur-md bg-white/95 shadow-2xl rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-500 mt-2">
              Bạn có chắc chắn muốn xóa hồ sơ khách thuê <span className="font-semibold text-rose-600">&ldquo;{name}&rdquo;</span> không?
              Hành động này sẽ xóa vĩnh viễn hợp đồng, tài khoản liên quan và giải phóng phòng về trạng thái trống (available).
            </DialogDescription>
          </DialogHeader>

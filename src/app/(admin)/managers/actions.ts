...
export async function editManager(
  id: string,
  data: { fullName: string; phone: string; password?: string; branchId?: string }
) {
  ...
  // 1. Cập nhật Auth User nếu có số điện thoại mới hoặc mật khẩu mới
  ...
  if (Object.keys(updateAuthData).length > 0 && userProfile?.phone) {

...
export function DeleteBranchButton({ id, name }: { id: number; name: string }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const errMsg = error instanceof Error ? error.message : 'Không thể xóa chi nhánh'
      toast.error(errMsg)
    } finally {
      setLoading(false)
    }
        ...
        <DialogContent className="sm:max-w-[420px] border border-gray-100/50 backdrop-blur-md bg-white/95 shadow-2xl rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-500 mt-2">
              Bạn có chắc chắn muốn xóa chi nhánh <span className="font-semibold text-rose-600">&ldquo;{name}&rdquo;</span> không?
              Hành động này không thể hoàn tác và toàn bộ liên kết phòng thuộc chi nhánh này sẽ bị ảnh hưởng.
            </DialogDescription>
          </DialogHeader>
